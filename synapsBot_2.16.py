    ...
    for word in words:
        if message.content.upper().startswith(word):
            if verified_role_id in [role.id for role in message.author.roles]:
                print("{0}: {1} activated a verified command".format(get_time(), user_name))
    ...
    for word in words:
        if message.content.upper().startswith(word):
            if admin_role_id in [role.id for role in message.author.roles]:
                print("{0}: {1} activated an admin command".format(get_time(), user_name))
                ...
                if admin_role_id not in [role.id for role in message.author.roles]:
                    print("{0}: {1} requested '.SEVERRULES' but wasn't an admin".format(get_time(), user_name))
                    await client.send_message(message.channel,
