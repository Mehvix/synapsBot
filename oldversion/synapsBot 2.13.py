...
async def on_server_join(member):
    ...
    client.send_message(discord.Object(id=notification_channel), "<@{}> joined the server :tada:".format(member))
...
async def on_message(message):
    ...
    if shut_up_role in [role.id for role in message.author.roles]:
        print("Told {} to shaddup".format(user_id))
        ...
        if message.content.upper().startswith(word):
            if verified_role_id in [role.id for role in message.author.roles]:
                ...
                if message.content.upper().startswith(".VERSION"):
                    await client.send_message(message.channel, "The bot is currently version `{}`"
                ...
                if message.content.upper().startswith(".SERVER"):
                    ...
                    if message.server.region == str("us-central"):
