    ...
    if reaction.message.channel.id != pokemon_channel:
        ...
        if emoji_used == formted_up:
            if reaction.message.author.id == user.id:
                ...
                print("GAVE 5 karma to {0} ({2}) for a reaction from {1} ({3})"
        ...
        if emoji_used == formted_up:
            if reaction.message.channel == notification_channel:
                if reaction.message.author.id == user.id:
                    ...
                    user_add_karma(reaction.message.author.id, -5)
                    print("REMOVED 5 karma to {0} ({2}) for a reaction from {1} ({3})"
                          .format(reaction.message.author, user, reaction.message.author.id, user.id))
            else:
...
async def on_message(message):
    ...
    if message.channel.id == pokemon_channel:
        print("{1}: DIDN'T given karma to {0} because they sent a message in the Pokemon channel"
              .format(message.author, fulltime))
    else:
        user_add_karma(user_id, 1)
        print("{3}: GAVE 1 karma to {0} ({1}) for a message in {2}"
              .format(user_name, user_id, message.channel.id, fulltime))
    ...
    # git joke
    if "GIT" in message.content.upper():
        print("{0}: {1} activated my 'GIT' card!".format(fulltime, user_name))
        memes = ["GUD", "JOB", "REKT", "SPOOKED"]
        for word in memes:
            if message.content.upper().startswith(word):
                if "GUD" in message.content.upper():
                    await client.send_message(message.channel, "git: 'gud' is not a git command. See 'git --help'.")
                if "JOB" in message.contontent.upper():
                    await client.send_message(message.channel, "git: 'job' is not a git command. See 'git --help'.")
                if "REKT" in message.contontent.upper():
                    await client.send_message(message.channel,
                                              "git: 'rekt' is not a git command. See 'git --help'.")
                if "SPOOKED" in message.contontent.upper():
                    await client.send_message(message.channel,
        ...
        if message.content.upper().startswith(word):
            if verified_role_id in [role.id for role in message.author.roles]:
                ...
                if message.content.upper().startswith(".CREATEPOLL"):
                    ...
                    await client.delete_message(message)
                    await client.send_message(message.channel, "{} Poll creation in progress..")
                    ...
                    if poll_options <= 1:
                        ...
                        if poll_options == 2:
                            ...
                            embed.add_field(name="1ï¸âƒ£ Option 1:",
                                            value=string.capwords(poll_option_1),
                                            inline=True)
                            embed.add_field(name="2ï¸âƒ£ Option 2:",
                                    ...
                                    if poll_options == 5:
                                        ...
                                        try:
                                            ...
                                            embed.add_field(name="2ï¸âƒ£ Option 2:",
                                                            ...
                                                            inline=True)
                                            embed.add_field(name="3ï¸âƒ£ Option 3:",
                                                            value=string
                                                            .capwords(poll_option_3),
                                                            inline=True)
                                            embed.add_field(name="4ï¸âƒ£ Option 4:",
                                                            value=string
                                                            .capwords(poll_option_4),
                                                            inline=True)
                                            embed.add_field(name="5ï¸âƒ£ Option 5:",
                                                            value=string
                                                            .capwords(poll_option_5),
                                                            inline=True)
                                            embed.add_field(name="6ï¸âƒ£ Option 6:",
                                                            value=string
                ...
                if message.content.upper().startswith(".WHOIS"):
                    ...
                    if message.content[7:] == "":
                        ...
                        try:
                            ...
                            embed = discord.Embed(title="Username:", description=full_user_name, color=0x1abc9c)
        ...
        if message.content.upper().startswith(".SERVERRULES"):
            ...
            embed.add_field(name="ðŸ”¸ Getting Member:",
                            value="Read the rules above and type '.accept' in here. If for whatever reason it doesnt "
                                  "work, contact  an {}.".format(admin_role_name), inline=True)
            await client.send_message(message.channel, embed=embed)
