...
async def on_message(message):
    ...
    if message.channel == pokemon_channel:
        print("DIDN'T given karma to {0} because they sent a message in the Pokemon channel")
    else:
        ...
        if message.content.upper().startswith(word):
            if verified_role_id in [role.id for role in message.author.roles]:
                ...
                if message.content.upper().startswith(".BETA"):
                    ...
                    msg = await client.wait_for_message(content=".allow", timeout=2)
                    if msg is None:
