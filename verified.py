    ...
    async def on_message(self, message):
        ...
        if settings.verified_role_id in [role.id for role in message.author.roles]:
            ...
            if message.content.upper().startswith(".BETA"):
                ...
                await self.client.send_message(message.channel, "Hey <@!196355904503939073>, <@{}> wants beta access.\nType"
                                                                " `.allow` to send them an invite".format(user_id))
                msg = await self.client.wait_for_message(content=".allow")
                if msg is None:
                    await self.client.send_message(
                        message.channel, "<@!196355904503939073> didn't respond in time :(. Please try another time.")
                else:
                    if msg.author.id == "196355904503939073":  # My Discord ID
                        ...
                        await self.client.send_message(message.channel, "<@{}> was accepted into the beta testing server! "
                                                                        ":tada:".format(user_id))
                    else:
            ...
            if message.content.upper().startswith(".WHOIS"):
                ...
                if message.content[7:] is None:
                    ...
                    try:
                        ...
                        embed = discord.Embed(title="Username:", description=full_user_name, color=settings.embed_color)
                        ...
                        print("{0}: {1} requested '.WHOIS' but they DIDN'T exist".format(curtime.get_time(), user_name))
            ...
            if message.content.upper().startswith(".BANLIST"):
                ...
                if not ban_list:
                    ...
                    await self.client.send_message(message.channel, "**Ban list:** \n{}".format("\n".join(pretty_list)))
            ...
            if message.content.upper().startswith(".ROULETTE"):
                ...
                if message.content.upper().startswith(".ROULETTE OUTCOMES"):
                    ...
                    await self.client.send_message(
                        message.channel, "How much would you like to bet? It must be between `10` and `250` and cannot be "
                    ...
                    if 10 <= bet_amount <= 250:
                        ...
                        await self.client.send_message(
                            message.channel, "What outcome would you like to bet on? The options are `zero`, `even`, or "
                                             "`odd`")
                        ...
                        if outcomes_formatted in outcomes:
                            ...
                            if msg.content == "It landed on `0`!":
                                ...
                                await self.client.send_message(david_dm, "Someone landed on 0 via roulette in `{}`".format(
                                    message.server.name))
                            ...
                            if spin == 0:
                                ...
                                if outcomes_formatted == "zero":
                                    ...
                                    await self.client.send_message(
                                        message.channel, "Winner! :tada: You quattuordecuple up on karma for a total of "
                                ...
                                if spin % 2 == 0:
                                    ...
                                    if outcomes_formatted == "even":
                                        ...
                                        await self.client.send_message(message.channel,
                                                                       "Winner! :tada: You doubled up on karma for a total "
                                                                       "of `{}`!".format(karma.get_karma(user_id)))
                                    else:
                                        await self.client.send_message(
                                            message.channel, "Sorry, better luck next time. You now have `{}` karma".format(
                                                karma.get_karma(user_id)))
            ...
            if message.content.upper().startswith(".BANNEDWORDS"):
                banned_words = settings.get_json("C:/Users/maxla/PycharmProjects/synapsBot remastered/banned_words.json")
                await self.client.send_message(
